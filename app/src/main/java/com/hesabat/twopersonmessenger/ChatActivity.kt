...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        adapter = MessageAdapter(Session.uid(this), { deleteDialog(it) }, { v -> openCall(v) })
    ...
    private fun showMenu() {
        PopupMenu(this,b.settingsBtn).apply {
            menu.add("Zənglər"); menu.add("Tənzimləmələr"); menu.add("Söhbəti bu cihazda təmizlə")
            setOnMenuItemClickListener { when(it.title.toString()) { "Zənglər" -> startActivity(Intent(this@ChatActivity,CallsActivity::class.java)); "Tənzimləmələr" -> startActivity(Intent(this@ChatActivity,SettingsActivity::class.java)); else -> clearLocalChat() }; true }; show()
        }
    }
    ...
    private fun load() {
        ...
        Api.get(q,Session.token(this)){ok,raw->
            ...
            runOnUiThread {
                ...
                val missed = CallHistory.list(this@ChatActivity).filter { it.direction=="incoming" && it.status=="missed" }.map { c -> Message(id=-(c.startedAt % 2000000000L).toInt().coerceAtLeast(1), sender_id=0, receiver_id=Session.uid(this@ChatActivity), type=if(c.type=="video") "call_missed_video" else "call_missed_audio", text="", created_at=java.text.SimpleDateFormat("HH:mm",java.util.Locale.getDefault()).format(java.util.Date(c.startedAt))) }
                val merged=(adapter.items + incoming + missed).associateBy{it.id}.values.sortedWith(compareBy<Message>{ if(it.id<0) Int.MAX_VALUE else it.id })
